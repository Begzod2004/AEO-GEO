...
<img src="https://cdn.simpleicons.org/probot/7C6BFF" width="76" height="76" alt="AEO.GEO logo" />
...
<!-- Typing banner -->
<a href="#-vision">
  <img src="https://readme-typing-svg.demolab.com?font=Geist+Mono&weight=600&size=22&pause=1200&color=7C6BFF&center=true&vCenter=true&width=720&lines=Search+is+becoming+conversation.;Your+brand+must+be+readable+by+machines.;Crawl+%E2%86%92+Structure+%E2%86%92+Optimize+%E2%86%92+Monitor+%E2%86%92+Deliver.;AEO.GEO+%E2%80%94+the+AI+Visibility+Operating+System." alt="Typing banner" />
</a>
...
<!-- Primary badges -->
<p>
  <img alt="Status" src="https://img.shields.io/badge/status-MVP%20live-7C6BFF?style=for-the-badge&labelColor=0B1220" />
  <img alt="License" src="https://img.shields.io/badge/license-MIT-22D3EE?style=for-the-badge&labelColor=0B1220" />
  <img alt="Backend Tests" src="https://img.shields.io/badge/tests-89%20passing-3FB950?style=for-the-badge&labelColor=0B1220" />
  <img alt="Lighthouse" src="https://img.shields.io/badge/lighthouse-95%20%2F%20100%20%2F%20100-F59E0B?style=for-the-badge&labelColor=0B1220" />
</p>
...
<p>
  ...
  <img alt="Celery" src="https://img.shields.io/badge/Celery-5.6-37814A?style=flat-square&logo=celery&logoColor=white" />
  ...
  <img alt="Qdrant" src="https://img.shields.io/badge/Qdrant-Vector%20DB-FF3E00?style=flat-square&logo=qdrant&logoColor=white" />
...
<!-- Nav buttons -->
<p>
  <a href="#-quick-start"><img src="https://img.shields.io/badge/%E2%96%B6%20Quick%20Start-7C6BFF?style=for-the-badge&labelColor=0B1220" alt="Quick Start" /></a>
  <a href="#-product-tour"><img src="https://img.shields.io/badge/%F0%9F%96%A5%20Live%20Demo-0B1220?style=for-the-badge&labelColor=0B1220" alt="Demo" /></a>
  <a href="#-architecture"><img src="https://img.shields.io/badge/%F0%9F%8F%9B%20Architecture-0B1220?style=for-the-badge&labelColor=0B1220" alt="Architecture" /></a>
  <a href="#-roadmap"><img src="https://img.shields.io/badge/%F0%9F%97%BA%20Roadmap-0B1220?style=for-the-badge&labelColor=0B1220" alt="Roadmap" /></a>
  <a href="#-license"><img src="https://img.shields.io/badge/%F0%9F%93%9C%20License-0B1220?style=for-the-badge&labelColor=0B1220" alt="License" /></a>
</p>
...
```mermaid
timeline
    title The Discovery Stack, Through Three Eras
    1998 - 2015 : Search Engine Optimization : Keywords, backlinks, ten blue links
    2016 - 2022 : Answer Boxes & Voice : Featured snippets, knowledge panels, assistants
    2023 - Now  : Answer Engine Optimization : LLM answers, citations, zero-click, RAG retrieval
    Next        : AI Visibility Operating Systems : Continuous, measured, self-optimizing brand presence in every model
```
...
```mermaid
flowchart LR
    U([👤 Customer asks an AI]) --> M{{🤖 Answer Engine}}
    M -->|reads structured, trusted sources| V["✅ A brand it can parse<br/>gets recommended + cited"]
    M -->|finds nothing machine-readable| X["❌ Your brand<br/>omitted or hallucinated"]
    style V fill:#0B2E1F,stroke:#3FB950,color:#E5E7EB
    style X fill:#2E0B14,stroke:#FB7185,color:#E5E7EB
    style M fill:#0B1220,stroke:#7C6BFF,color:#E5E7EB
```
...
flowchart TB
    subgraph Edge["🌍 Edge"]
        NGINX["Nginx reverse proxy<br/>TLS · routing · same-origin API"]
    end

    subgraph Clients["🖥 Clients"]
        LANDING["Landing<br/>Next.js SSG · en/ru/uz"]
        SPA["Dashboard SPA<br/>React + Vite + TS"]
    end

    subgraph Core["⚙️ Application Core — Django + DRF"]
        API["REST API<br/>JWT · RBAC · tenant isolation"]
        ADMIN["Django Admin +<br/>Platform Panel"]
    end

    subgraph Async["🛰 Async Workers"]
        WORKER["Celery Worker<br/>crawl · embed · scan · schema"]
        BEAT["Celery Beat<br/>nightly score refresh"]
    end

    subgraph Data["💾 Data Layer"]
        PG[("PostgreSQL<br/>tenants · scores · audit")]
        REDIS[("Redis<br/>cache · broker · tokens")]
        QDRANT[("Qdrant<br/>per-org vectors")]
    end

    subgraph AI["🤖 AI Gateway (mock ⇄ live)"]
        GW["Provider Gateway"]
        OAI["OpenAI"]
        ANT["Anthropic"]
        GEM["Gemini"]
    ...
    classDef store fill:#0B1220,stroke:#22D3EE,color:#E5E7EB
    classDef svc fill:#0B1220,stroke:#7C6BFF,color:#E5E7EB
    class PG,REDIS,QDRANT store
    class API,ADMIN,WORKER,BEAT,GW svc
```
...
sequenceDiagram
    ...
    API->>R: store refresh jti (TTL)
    API-->>U: { access, refresh, user }

    U->>API: GET /api/organizations/{id}/dashboard/ (Bearer)
    API->>API: IsOrgMember → membership check (from URL, never body)
    API->>DB: query scoped to organization_id
    API-->>U: 200 latest + trend  ·  else 403

    U->>API: POST /api/auth/logout/ {refresh}
    API->>R: revoke jti
    Note over U,R: refresh now returns 401 — real logout
```
...
```mermaid
flowchart LR
    T([POST /crawl/]) --> C[Create CrawlResult<br/>status: pending]
    C --> Q{{Celery queue}}
    Q --> F[Fetch main page]
    F --> P[Parse title · meta · canonical]
    P --> R[Check robots.txt & sitemap.xml]
    R --> L[Classify internal/external links]
    L --> B[Probe links → broken?]
    B --> S[Compute performance score]
    S --> D[(Save meta · status: done)]
    style D fill:#0B2E1F,stroke:#3FB950,color:#E5E7EB
```
...
```mermaid
flowchart LR
    A([Any security-relevant action]) --> REC["audit.record(action, user, org, meta)"]
    REC --> TRY{write AuditLog}
    TRY -->|ok| LOG[(append-only AuditLog)]
    TRY -->|fails| WARN[log warning · never break the op]
    LOG --> PANEL[Platform Admin · Audit feed]
    LOG --> DJ[Django Admin · read-only]
    style LOG fill:#0B1220,stroke:#22D3EE,color:#E5E7EB
```
...
```mermaid
flowchart TD
    W["🌐 Website / Documents"] --> CR["🕷 Crawler / Extractor<br/>trafilatura · pdfplumber · python-docx"]
    CR --> CL["🧹 Cleaner<br/>strip nav / boilerplate"]
    CL --> CH["✂️ Chunker<br/>sentence-aware, overlap"]
    CH --> EM["🧮 Embedding<br/>1536-dim · mock ⇄ OpenAI"]
    EM --> VDB[("🗄 Vector DB<br/>Qdrant org_{id}")]
    VDB --> KG["🧬 Knowledge Graph<br/>per-tenant retrieval"]
    KG --> OPT["📡 LLM Optimization<br/>grounded schema · llms.txt · profile"]
    OPT --> MON["🔭 Monitoring<br/>prompts × providers → signals"]
    MON --> AN["📊 Analytics & Scoring<br/>6 transparent scores + trend"]
    AN -->|recommendations| OPT

    classDef n fill:#0B1220,stroke:#7C6BFF,color:#E5E7EB
    classDef store fill:#0B1220,stroke:#22D3EE,color:#E5E7EB
    class W,CR,CL,CH,EM,KG,OPT,MON,AN n
    class VDB store
```
...
flowchart LR
    ...
    subgraph Store["Per-tenant Store"]
        E --> QP[(Qdrant points<br/>payload: org_id, doc_id, text)]
        C --> PGR[(Postgres Chunk rows<br/>Qdrant point id link)]
    end
    subgraph Retrieve["Retrieval / RAG"]
        Q[Query] --> QE[Embed query]
        QE --> SR{Cosine search<br/>top-k, org-scoped}
        QP --> SR
        SR --> CTX[Grounded context]
        CTX --> GEN[Grounded generation<br/>FAQ · answers · llms.txt]
    end
    style QP fill:#0B1220,stroke:#22D3EE,color:#E5E7EB
    style GEN fill:#0B2E1F,stroke:#3FB950,color:#E5E7EB
```
...
```mermaid
flowchart LR
    DEV[git push] --> CI{{GitHub Actions<br/>89 tests + 2 builds}}
    CI -->|green| REG[Build prod images]
    REG --> VPS[VPS · docker compose -f docker-compose.prod.yml]
    VPS --> N[Nginx :80/:443]
    N --> L[Landing · LANDING_DOMAIN]
    N --> A[Dashboard + API · APP_DOMAIN]
    N --> TLS[Let's Encrypt / certbot]
    style CI fill:#0B1220,stroke:#7C6BFF,color:#E5E7EB
```
...
```mermaid
gantt
    title AEO.GEO — Delivery Roadmap
    dateFormat YYYY-MM-DD
    axisFormat %b

    section Phase 1 · MVP ✅
    Auth · tenancy · RBAC            :done, p1a, 2026-07-01, 3d
    Crawler · KB · vector search     :done, p1b, after p1a, 3d
    Schema · monitoring · scoring    :done, p1c, after p1b, 3d
    Landing · admin · AI delivery    :done, p1d, after p1c, 4d

    section Phase 2 · Live & Grow
    Live AI providers (keys)         :active, p2a, 2026-07-20, 10d
    Billing (Stripe / Payme / Click) :p2b, after p2a, 14d
    Competitor Intelligence          :p2c, after p2a, 14d
    Content Studio · Automation UI   :p2d, after p2b, 20d

    section Phase 3 · Scale
    CRM · Citation Manager           :p3a, 2026-09-01, 30d
    White Label · Agency console     :p3b, after p3a, 30d
    Public API · SDKs · Webhooks     :p3c, after p3a, 30d

    section Phase 4 · Platform
    AI Agents (autonomous fixes)     :p4a, 2026-11-01, 45d
    Global Knowledge Graph           :p4b, after p4a, 45d
    Kubernetes · Terraform · SSO     :p4c, after p4a, 30d
```
...
|:---:|:---:|
| <img src="https://placehold.co/640x400/050816/7C6BFF?text=Landing+%E2%80%94+Knowledge+Graph+Hero" width="100%"/> | <img src="https://placehold.co/640x400/080B12/22D3EE?text=Dashboard+%E2%80%94+6+Scores+%2B+Trend" width="100%"/> |

| Platform Admin | AI Delivery (Schema) |
|:---:|:---:|
| <img src="https://placehold.co/640x400/0B1220/3FB950?text=Admin+%E2%80%94+Health+%2B+Issues+%2B+Retry" width="100%"/> | <img src="https://placehold.co/640x400/0B1220/F59E0B?text=Deliver+to+AI+%E2%80%94+llms.txt+%2B+JSON-LD" width="100%"/> |
...
<img alt="GitHub" src="https://img.shields.io/badge/GitHub-Begzod2004%2FAEO--GEO-181717?style=for-the-badge&logo=github&logoColor=white" />
<img alt="Email" src="https://img.shields.io/badge/Early%20Access-join%20the%20waitlist-7C6BFF?style=for-the-badge&logo=maildotru&logoColor=white" />
...
## <img src="https://cdn.simpleicons.org/probot/7C6BFF" width="22" /> AEO.GEO
...
<img alt="Made with Django" src="https://img.shields.io/badge/Made%20with-Django%20%C2%B7%20React%20%C2%B7%20Qdrant-0B1220?style=flat-square" />
<img alt="Built for the AI era" src="https://img.shields.io/badge/Built%20for-the%20AI%20era-7C6BFF?style=flat-square" />
<img alt="Star" src="https://img.shields.io/badge/%E2%AD%90-star%20if%20this%20is%20the%20future-F59E0B?style=flat-square" />
